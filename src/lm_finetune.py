...
def finetune(rank, args):
    ...
    if args.load is not None and args.load.endswith("pt"):
        model = (
            LinearizedLanguageModel.load(args.load)
            if linearized_finetuning
            else LanguageModel.load(args.load)
        )
    else:
        print("Building language model")
        model = (
            LinearizedLanguageModel(args, keep_lang=False) #TODO: Create class
            if linearized_finetuning
            else LanguageModel(args) #TODO: Create class
        )
    ...
    data_collator = get_data_collator(tokenizer) #TODO: Create class: DONE
    compute_metrics = get_compute_metrics() #TODO: Create Class: DONE

    dataset = get_lm_dataset( #TODO: Create Class: DONE
        train_dataset,
        tokenizer,
        #preprocess_fn,
